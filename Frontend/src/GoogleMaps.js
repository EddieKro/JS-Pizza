function initialize() {
    var mapProp = {
        center: new google.maps.LatLng(50.464320, 30.518141),
        zoom: 11
    };
    var point = new google.maps.LatLng(50.464320, 30.518141);
    ...
    google.maps.event.addListener(map, 'click', function (me) {
        ...
        geocodeLatLng(coordinates, function (err, address) {
            if (!err) {
                ...
                console.log("Вибачте,даної адреси не існує")
            }
        })
    });
...
function geocodeLatLng(latlng, callback) {
    ...
    geocoder.geocode({'location': latlng}, function (results, status) {
        if (status === google.maps.GeocoderStatus.OK && results[1]) {
            ...
            callback(new Error("Sorry, can't find address"));
        }
    });
}
...
function geocodeAddress(address, callback) {
    ...
    geocoder.geocode({'address': address}, function (results, status) {
        if (status === google.maps.GeocoderStatus.OK && results[0]) {
            ...
            callback(new Error("Sorry,can`t find the address"));
        }
    });
}
